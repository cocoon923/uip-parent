...
import com.ailife.uip.core.util.HTMLLEVEL;
	...
	private static List<ItemRelat> generateInterParamRelat(Inter inter) {
		...
		if (inter != null) {
			for (Param param : inter.getParams()) {
				itemRelatList.add(new ItemRelat(ID.getNewId(), inter.getSeq(), param.getSeq(), Inter.class, Param.class));
			}
		}
